...
void img::EasyImage::draw_zbuff_line(unsigned int x0, unsigned int y0, unsigned int x1, unsigned int y1,
                                     Color color, std::function<bool(int, int, double)> zBufferChecker)
{
    ...
    else
    {
        ...
        if (-1.0 <= m && m <= 1.0) {
            for (unsigned int i = 0; i <= (x1 - x0); i++) {
                t = (double)i / (x1 - x0);
                if (zBufferChecker(x0 + i, (unsigned int) round(y0 + m * i), t)) {
                    (*this)(x0 + i, (unsigned int) round(y0 + m * i)) = color;
                }
            }
        } else if (m > 1.0) {
            for (unsigned int i = 0; i <= (y1 - y0); i++) {
                t = (double)i / (y1 - y0);
                if (zBufferChecker((unsigned int) round(x0 + (i / m)), y0 + i, t)) {
                    (*this)((unsigned int) round(x0 + (i / m)), y0 + i) = color;
                }
            }
        } else if (m < -1.0) {
